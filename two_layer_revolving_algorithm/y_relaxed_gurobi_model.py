'''
File: y_relaxed_gurobi_model.py
Project: Integrated_Picking_and_Sorting_Model
Description: 
----------
relaxed all variable except y.
----------
Author: 626
...
class yRelaxedGurobiModel:
    ...
    def build_gurobi_model(self, model):
        ...
        # 添加约束条件
        # 0. 最大完成时间约束
        model.addConstrs( FT >= self.Variable.T[i] for i in self.N)
        # picking 约束条件
        # 1. 车辆的流平衡约束
        ...
        # 3. 同一个任务用同一个车
        # model.addConstrs( self.Variable.passX[i,k] - self.Variable.passX[j,k] >= M * (x[i,j] - 1) for i in self.N for j in self.N for k in self.K)
        # model.addConstrs( self.Variable.passX[i,k] - self.Variable.passX[j,k] <= M * (1 - x[i,j]) for i in self.N for j in self.N for k in self.K)
        ...
        # 6. 时间约束
        # model.addConstrs( M * (1 - x[i,j]) + self.Variable.T[j] >= self.Variable.T[i] + self.timeMatrix[i][j] + self.nodes[i]["serviceTime"] for i in self.N for j in (self.P1 + self.P2 + self.D1 + self.D2) if i!=j)
        ...
        model.addConstrs( self.Variable.Ts[j,p] - self.Variable.Te[i,p] >= M * (self.Variable.f[i,j,p] + y[i,p] + y[j,p] - 3) for i in range(self.n) for j in range(i+1,self.n) for p in range(self.P))
        model.addConstrs( self.Variable.Ts[i,p] - self.Variable.Te[j,p] >= M * ((1- self.Variable.f[i,j,p]) + y[i,p] + y[j,p] - 3) for i in range(self.n) for j in range(i+1,self.n) for p in range(self.P))
        # 12. 只有订单o被分配给了拣选站p & 料箱i属于订单o，那么料箱i一定能被分配给拣选站p
        model.addConstrs( self.IO[i][o] * self.Variable.z[o,p] <= y[i,p] for i in range(self.n) for p in range(self.P) for o in range(self.O))
        # 13. 所有任务都要被完成
        ...
        model.addConstrs( self.Variable.Te[i,p] == self.Variable.Ts[i,p] + self.picking_time * y[i,p] for i in range(self.n) for p in range(self.P))
        # 15. 当料箱i不去拣选站p时，Ts=Ta
        model.addConstrs( self.Variable.Ts[i,p] <= self.Variable.Ta[i,p] + M * y[i,p] for i in range(self.n) for p in range(self.P))
        ...
        model.addConstrs( self.Variable.toe[o] - self.Variable.Te[i,p] >= (self.IO[i][o] + y[i,p] + self.Variable.z[o,p] - 3) * self.bigM for o in range(self.O) for i in range(self.n) for p in range(self.P))
        model.addConstrs( self.Variable.tos[o] - self.Variable.Ts[i,p] <= (3 - self.IO[i][o] - y[i,p] - self.Variable.z[o,p]) * self.bigM for o in range(self.O) for i in range(self.n) for p in range(self.P))
        model.update()
...
if __name__ == "__main__":
    ...
    solver = yRelaxedGurobiModel(problem, variable)
