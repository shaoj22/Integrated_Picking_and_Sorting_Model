'''
File: z_relaxed_gurobi_model.py
Project: Integrated_Picking_and_Sorting_Model
Description: 
----------
relaxed all variable except z.
----------
Author: 626
...
class zRelaxedGurobiModel:
    def __init__(self, integrated_instance, Variable, time_limit=None, init_flag=False):
        """
        init the zRelaxedGurobiModel with inputting instance and Variable.
    ...
    def build_gurobi_model(self, model):
        ...
        model.addConstrs( self.IO[i][o] * z[o,p] <= self.Variable.y[i,p] for i in range(self.n) for p in range(self.P) for o in range(self.O))
        # 13. 所有任务都要被完成
        ...
        model.addConstrs( self.Variable.d1[o1,o2] >= self.Variable.c1[o1,o2] + z[o1,p] +z[o2,p] - 2 for o1 in range(self.O) for o2 in range(self.O) for p in range(self.P))
        # model.addConstrs( gp.quicksum( d1[o1,o2] for o2 in range(self.O)) <= 8 for o1 in range(self.O))
        # 添加订单任务分拨上墙与任务分配和机器人调度的衔接约束条件
        model.addConstrs( self.Variable.toe[o] - self.Variable.Te[i,p] >= (self.IO[i][o] + self.Variable.y[i,p] + z[o,p] - 3) * self.bigM for o in range(self.O) for i in range(self.n) for p in range(self.P))
        model.addConstrs( self.Variable.tos[o] - self.Variable.Ts[i,p] <= (3 - self.IO[i][o] - self.Variable.y[i,p] - z[o,p]) * self.bigM for o in range(self.O) for i in range(self.n) for p in range(self.P))
        model.update()
...
if __name__ == "__main__":
    ...
    solver = zRelaxedGurobiModel(problem, variable)
