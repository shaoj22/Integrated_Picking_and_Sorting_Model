...
class xRelaxedGurobiModel:
    ...
    def build_gurobi_model(self, model):
        ...
        # 5. 载重约束
        model.addConstrs( self.Q * (1 - x[i, j]) + self.Variable.Q[j] >= self.Variable.Q[i] + self.nodes[i]["demand"]  for i in self.N for j in (self.P1 + self.P2 + self.D1 + self.D2) if i!=j)
        # model.addConstrs( (x[i, j] == 1) >> (self.Variable.Q[j] >= self.Variable.Q[i] + self.nodes[i]["demand"]) for i in self.N for j in (self.P1 + self.P2 + self.D1 + self.D2) if i!=j)
        ...
        # model.addConstrs( (x[i, j] == 1) >> (self.Variable.T[j] >= self.Variable.T[i] + self.timeMatrix[i][j] + self.nodes[i]["serviceTime"]) for i in self.N for j in (self.P1 + self.P2 + self.D1 + self.D2) if i!=j)
