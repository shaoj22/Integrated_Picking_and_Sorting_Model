...
class zRelaxedGurobiModel:
    ...
    def build_gurobi_model(self, model):
        ...
        model.addConstrs( toe[o] - Te[i,p] >= (self.IO[i][o] + self.variable.y[i,p] + z[o,p] - 3) * self.bigM for o in range(self.O) for i in range(self.n) for p in range(self.P))
        model.addConstrs( tos[o] - Ts[i,p] <= (3 - self.IO[i][o] - self.variable.y[i,p] - z[o,p]) * self.bigM for o in range(self.O) for i in range(self.n) for p in range(self.P))
        model.update()
        ...
        if model.status == 2:
            ...
            for o in range(self.O):
                ...
                for p in range(self.P):
                    var_name = f"z[{o},{p}]"
                    z_o_p = model.getVarByName(var_name).X
                    each_z.append(z_o_p)
                solution_z.append(each_z)
        objVal = model.objVal
...
if __name__ == "__main__":
    ...
    # t_sum = sum(sum(row) for row in y_val)
    # print(t_sum)
    # print(problem.sumIO)
    # print(y_val)
