...
class yRelaxedGurobiModel:
    def __init__(self, integrated_instance, variable, time_limit=None, init_flag=False):
        """
        init the yRelaxedGurobiModel with inputting instance and variable.
    ...
    def build_gurobi_model(self, model):
        ...
        # picking 约束条件
        # 1. 车辆的流平衡约束
        model.addConstrs( gp.quicksum( self.variable.x[i,j] for j in self.N if j != i ) == gp.quicksum( self.variable.x[j,i] for j in self.N if j != i ) for i in self.N)
        # 2. 车辆要完成所有任务
        ...
        # 5. 载重约束
        model.addConstrs( Q[j] >= Q[i] + self.nodes[i]["demand"] - self.Q * (1 - self.variable.x[i, j]) for i in self.N for j in (self.P1 + self.P2 + self.D1 + self.D2) if i!=j)
        ...
        model.addConstrs( y[i,p] >= self.IO[i][o] * self.variable.z[o,p] for i in range(self.n) for p in range(self.P) for o in range(self.O))
        # 13. 所有任务都要被完成
        ...
        model.addConstrs( toe[o] - Te[i,p] >= (self.IO[i][o] + y[i,p] + self.variable.z[o,p] - 3) * self.bigM for o in range(self.O) for i in range(self.n) for p in range(self.P))
        model.addConstrs( tos[o] - Ts[i,p] <= (3 - self.IO[i][o] - y[i,p] - self.variable.z[o,p]) * self.bigM for o in range(self.O) for i in range(self.n) for p in range(self.P))
        model.update()

    def set_init_solution(self, model):
        """ set init solution for model """
        y_val = self.variable.y
        for i in range(self.integrated_instance.nodeNum):
            for p in range(self.integrated_instance.P):
                model.getVarByName("y[{},{}]".format(i,p)).start = y_val[i,p]
        model.update()
        ...
        if model.status == 9 or model.status == 2:
            ...
            for i in range(self.n):
                ...
                for p in range(self.P):
                    var_name = f"y[{i},{p}]"
                    y_i_p = model.getVarByName(var_name).X
                    each_y.append(y_i_p)
                solution_y.append(each_y)
        objVal = model.objVal
        objBound = model.objBound

        return objVal, objBound, solution_y
