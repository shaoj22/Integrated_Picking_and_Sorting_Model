...
class SortingGreedyRepair(Operator):
    ...
    def backupon(self, best_solution, best_obj, cur_solution, break_o_list, cur_p):
        if cur_p >= len(break_o_list):
            cur_obj, _ = efficient_integrated_evaluate(self.instance, cur_solution['picking'], cur_solution['sorting'])
            if cur_obj < best_obj:
                best_obj = cur_obj
    ...
    def set(self, solution, break_info):
        ...
        best_obj, _ = efficient_integrated_evaluate(self.instance, best_solution['picking'], best_solution['sorting']) 
