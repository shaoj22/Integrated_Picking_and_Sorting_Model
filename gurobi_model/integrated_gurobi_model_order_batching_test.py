'''
File: integrated_gurobi_model_order_batching_test.py
Project: Integrated_Picking_and_Sorting_Model
Description: 
----------
use the heuristic strategy algorithm to get the y and z,\
then fixed the y and z, and test the efficiency of the model accelerated or not.
----------
Author: 626
...
from order_batching_algorithm.heuristic_algorithm import get_order_batching_fixed_solution_by_heuristic_algorithm
...
class Integrated_Gurobi_Model(Picking_Gurobi_Model):
    def __init__(self, instance, time_limit=None):
        ...
        self.bigM = 1000
        self.y, self.z = get_order_batching_fixed_solution_by_heuristic_algorithm(self.n, self.P, self.O, self.IO)
    ...
    def build_model(self, MODEL):
        ...
        MODEL.addConstrs( Ts[j,p] - Te[i,p] >= M * (f2[i,j,p] + self.y[i,p] + self.y[j,p] - 3) for i in range(self.n) for j in range(i+1,self.n) for p in range(self.P))
        MODEL.addConstrs( Ts[i,p] - Te[j,p] >= M * ((1- f2[i,j,p]) + self.y[i,p] + self.y[j,p] - 3) for i in range(self.n) for j in range(i+1,self.n) for p in range(self.P))
        # 约束条件5：只有订单o被分配给了拣选站p & 料箱i属于订单o，那么料箱i一定能被分配给拣选站p
        # MODEL.addConstrs( self.IO[i][o] * self.z[o,p] <= self.y[i,p] for i in range(self.n) for p in range(self.P) for o in range(self.O))
        # 约束条件6：所有任务都要被完成
        MODEL.addConstrs( gp.quicksum( self.z[o,p] for p in range(self.P)) == 1 for o in range(self.O))
        MODEL.addConstr( gp.quicksum( self.y[i,p] for i in range(self.n) for p in range(self.P)) == self.sumIO)
        # 关于1个料箱可以去多个拣选站的约束
        # --------------------------------------------------------------------------------------------------------------
        # 关于料箱i在拣选站p的结束拣选时间
        # 约束条件1：料箱i的结束拣选时间一定大于等于它的开始拣选时间（当yip=0时）
        MODEL.addConstrs( Te[i,p] == Ts[i,p] + self.picking_time * self.y[i,p] for i in range(self.n) for p in range(self.P))
        # 约束条件3：当料箱i不去拣选站p时，Ts=Ta
        MODEL.addConstrs( Ts[i,p] <= Ta[i,p] + M * self.y[i,p] for i in range(self.n) for p in range(self.P))
